 ...
 *                                                                                 *
 * Copyright 2007-2016 by Shengalts Aleksander aka Instructor (Shengalts@mail.ru)  *
 *                                                                                 *
...
BOOL AkelEditInit(HINSTANCE hInst)
{
    return AE_RegisterClassW(hInst, FALSE);

}

BOOL AkelEditTerm(HINSTANCE hInst)
{
    return AE_UnregisterClassW(hInst);
    //OleUninitialize();
}
...
HWND AE_CreateEditWindow(HWND hWndParaent, HINSTANCE hInstance, CREATESTRUCTW* csw)
{
    HWND hWnd = NULL;

    if (IsWindowUnicode(hWndParaent)) /* we only support unicode enabled window */
    {
        hWnd = CreateWindowExW(0, AES_AKELEDITW, NULL, WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN | WS_VSCROLL,
            CW_USEDEFAULT, 0, CW_USEDEFAULT, 0, hWndParaent, NULL, hInstance, csw);
    }
...
LRESULT CALLBACK AE_EditProc(AKELEDIT *ae, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  switch (uMsg)
  {
    ...
    case WM_MOUSEMOVE:
        PostMessage(ae->hWndParent, WM_AKELPAD_MESSAGE, WM_MOUSEMOVE, 0);
    case WM_LBUTTONDOWN:
    case WM_LBUTTONUP:
  ...
  if (uMsg >= WM_KEYFIRST && uMsg <= WM_KEYLAST)
  {
    ...
    else if (uMsg == WM_CHAR)
    {
      ...
      if (wParam == VK_RETURN)
      {
          LPARAM lp = lParam;   // hacking: wochatyou@gmail.com
          if (VK_CONTROL == lp)
          {
              AE_EditKeyReturn(ae);
          }
          ...
          {
              SendMessage(ae->hWndParent, WM_AKELPAD_MESSAGE, WM_AKELPAD_RETURN, 0);
          }
          return 1;
      }
             ...
             uMsg == WM_SYSKEYUP)
    {
      ...
      if (uMsg == WM_SYSCHAR)
      {
          ...
          LPARAM lp = lParam;
          ae->nAltChar=AEAC_NONE;
          // hacking wochatyou@gmail.com
          if (VK_RETURN == wp)
          {
              SendMessage(ae->hWndEdit, WM_CHAR, VK_RETURN, VK_CONTROL);
          }
      }
    }
  }
...
AKELEDIT* AE_CreateWindowData(HWND hWnd, CREATESTRUCTA *cs, AEEditProc lpEditProc)
{
  ...
  if (ae=AE_StackWindowInsert(&hAkelEditWindowsStack))
  {
    ...
    // short-cut
    ae->popt->dwOptions |= AECO_MULTILINE; // default is multiline wochatyou@gmail.com
    ae->bHScrollShow = FALSE;
    ae->bVScrollShow = TRUE;
    ae->ptxt->dwWordWrap = 1;
