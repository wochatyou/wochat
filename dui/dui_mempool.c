	...
	Size maxBlockSize)
{
	...
	if (minContextSize != 0)
		firstBlockSize = PGMax(firstBlockSize, minContextSize);
	else
		firstBlockSize = PGMax(firstBlockSize, initBlockSize);
...
void* palloc(MemoryContext cxt, Size size)
{
	...
	//AssertNotInCriticalSection(cxt);
	...
	ret = cxt->methods->alloc(cxt, size);
